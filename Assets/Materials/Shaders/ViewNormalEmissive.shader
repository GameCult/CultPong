...
Shader "Aetheria/View Dependent Emissive" {
	Properties {
		...
		_OffsetTex ("Offset Texture", 2D) = "grey" {}
		_Offset ("Offset Distance", Float) = 0
		_Speed ("Morph Speed", Float) = 0
		//_BumpPower ("Bump Power", Range (0.01,5)) = 1
		//_Rotation ("Normal Rotation", Float) = 0
		_Glossiness ("Smoothness", Range(0,1)) = 0.5
		_MetallicGlossMap("MetalGloss", 2D) = "white" {}
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
            ...
            float sinX = sin ( _Time.y*_Speed );
            float cosX = cos ( _Time.y*_Speed );
            float sinY = sin ( _Time.y*_Speed );
...
/*
			...
			float2 Offset = mul ( tex2D(_OffsetTex, IN.uv_MainTex + mul ( (tex2D(_OffsetTex, IN.uv_MainTex).rg-(.5).xx)*_Offset, rotationMatrix)).rg, rotationMatrix)*_Offset;

			// Metallic and smoothness come from a texture
			float2 metalSmooth = tex2D (_MetallicGlossMap, IN.uv_MainTex + Offset).ra;
